...
let migrationsPath      = path.join(process.env.PWD, 'migrations');
var backup_dump         = require(migrationsPath+ "/_current_bak.json");
var current_dump        = require(migrationsPath+ "/_current.json");
...
const getMigration = function(actions) 
{
    ...
    for (let _i in actions)
    {
        ...
        var droppedTable = action.tableName;
...
let resUp =`{ fn: "createTable", params: [
    ...
    ${JSON.stringify(action.options)}
            ...
            case 'dropTable':
            {
                let resUp = `{ fn: "dropTable", params: ["${action.tableName}"] }`;
                commandsUp.push(resUp);
                
                consoleOut.push(`dropTable "${action.tableName}"`);
let res =`{ fn: "createTable", params: [
    "${action.tableName}",
    ${getAttributes(backup_dump["tables"][droppedTable]["schema"])},
    {}
] }`;
...
let resUp = `{ fn: "addColumn", params: [
    ...
    ${propertyToStr(action.options)}
                ...
                let res = `{ fn: "removeColumn", params: ["${action.tableName}", "${action.attributeName}"] }`;
                commandsDown.push(res);
            }
            break;
            ...
            case 'removeColumn':
            {
                let resUp = `{ fn: "removeColumn", params: ["${action.tableName}", "${action.columnName}"] }`;
                commandsUp.push(resUp);
                
                consoleOut.push(`removeColumn "${action.columnName}" from table "${action.tableName}"`);

let res = `{ fn: "addColumn", params: [
    "${action.tableName}",
    "${action.columnName}",
    ${propertyToStr(current_dump["tables"][action.tableName]["schema"][action.columnName])}
] }`;
...
{
    let _commandsUp = "var migrationCommandsUp = [ \n" + migration.commandsUp.join(", \n") +' \n];\n';
    let _commandsDown = "var migrationCommandsDown = [ \n" + migration.commandsDown.join(", \n") +' \n];\n';
    ...
    up: function(queryInterface, Sequelize)
    {
        ...
        return new Promise(function(resolve, reject) {
            function next() {
                ...
                {
                    let command = migrationCommandsUp[index];
    ...
    down: function(queryInterface, Sequelize)
    {
        ...
        return new Promise(function(resolve, reject) {
            function next() {
                ...
                {
                    let command = migrationCommandsDown[index];
                    console.log("[#"+index+"] execute: " + command.fn);
                    index++;
                    queryInterface[command.fn].apply(queryInterface, command.params).then(next, reject);
                }
